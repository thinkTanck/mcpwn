...
- **Pre-commit / pre-push** — husky + lint-staged + commitlint. **Pre-push is a fast local gate that validates the *commit being pushed*, not the working tree**: it checks out the pushed SHA into an isolated worktree (deps reused, never reinstalled), runs a fast **typecheck** there, and fails closed — so a dirty tree can't cry wolf, uncommitted work is never stashed or at risk, and it stays fast enough that it is never worth bypassing. It is deliberately *not* the full gate; **CI (which runs on every push) is the authoritative gate** (install → lint → typecheck → unit `--coverage` → build → e2e + axe → Lighthouse CWV → `impeccable detect` → `audit-ci`). "Done" for any screen = typecheck **and** unit **and** `detect` clean **and** measured numbers (type + CWV + axe), not tests alone.
