...
/**
 * BYOK endpoint validation is a SECURITY control, not a formatting nicety: the
...
describe('ByokEndpointSchema', () => {
  it('parses to the canonical href', () => {
    expect(ByokEndpointSchema.parse('  https://agent.example/mcp ')).toBe(
      'https://agent.example/mcp',
    );
  });

  it('fails with the user-readable reason', () => {
    const result = ByokEndpointSchema.safeParse('http://agent.example/mcp');
  ...
  it('rejects an absurdly long value before parsing it', () => {
    expect(ByokEndpointSchema.safeParse(`https://a.example/${'x'.repeat(3000)}`).success).toBe(
      false,
    );
  });
});
