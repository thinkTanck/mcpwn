...
| L9 | **The live-run allowance is decided but NOT implemented** ([ADR-0007](docs/adr/0007-access-and-cost-model.md)). On `main` **no cap is enforced at all** — `countRunsSince` has no caller. PR #80 (unmerged) implements a *different* model: `LIVE_RUN_CAP` (20) per `LIVE_RUN_WINDOW_HOURS` (24), a rolling window that ADR-0007 supersedes. Reconciling means counting from account creation rather than a window, collapsing the two env vars into one lifetime allowance, and defaulting it to ~3. | Until this lands, no UI copy or doc may state a specific free-run count, because the app would not honour it. |
