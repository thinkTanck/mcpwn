 ...
 * `McpTargetError`s. Nothing here ever puts the BYOK key in an error message.
 */
...
export interface McpTransportOptions {
  /** Validated BYOK endpoint (see `ByokEndpointSchema`). */
  endpoint: string;
  /** BYOK key, sent as `Authorization: Bearer`. Never logged, never persisted. */
  apiKey?: string;
