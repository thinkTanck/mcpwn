...
export function CommandDeck({ pathname, fleet }: { pathname: string; fleet: FleetStatusData }) {
  return (
    <nav
      ...
      className="sticky top-[62px] hidden h-[calc(100dvh-62px)] shrink-0 flex-col gap-1 self-start overflow-y-auto border-r border-line bg-gradient-to-b from-[var(--scrim-deck-top)] to-[var(--scrim-deck-bottom)] p-3 min-[760px]:flex min-[760px]:w-[72px] min-[1100px]:w-[236px]"
    >
      <div className="hidden px-3 pb-2.5 pt-1.5 font-mono text-[12px] tracking-[0.16em] text-ink-faint min-[1100px]:block">
        COMMAND DECK
      </div>
      {NAV_ITEMS.map((item) => (
        <NavLink
