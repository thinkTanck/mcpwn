 ...
 * (ASI02), sitting where the sentinel-core globe used to. It reads as a live
 * replay of the actual finding rather than decoration: the agent's steps stream
 * down a signal rail and the breach indicator lights at the offending `read_file`
 * step (`s6`), the same step the SampleTrailer badges.
 *
  ...
  return (
    ...
    >
      <div aria-hidden="true" className="mb-3 flex items-center justify-between gap-2">
        ...
        <span className="inline-flex items-center gap-1.5 font-mono text-[12px] uppercase tracking-[0.12em] text-ink-faint">
          <span className="h-1.5 w-1.5 rounded-full bg-nominal shadow-glow-nominal hero-live-dot" />
          live
        </span>
      </div>

      <ol aria-hidden="true" className="flex flex-col gap-[7px]">
        {steps.map((step, i) => {
            ...
            >
              <span className="relative flex h-2.5 w-2.5 shrink-0 items-center justify-center">
                <span
                  ...
                  style={{
                    background: breach
                      ? 'radial-gradient(circle, color-mix(in srgb, var(--red-400) 80%, transparent), transparent 70%)'
                ...
                <span
                  className={cn(
                    ...
                    breach && 'bg-breach shadow-glow-breach hero-breach-dot',
                  )}
                  style={breach ? undefined : { background: stepColorToken(step.type) }}
                />
              </span>
              <span className="w-[22px] shrink-0 font-mono text-[12px] tabular-nums text-ink-muted">
                {id}
              </span>
              ...
              {breach && (
                <span className="shrink-0 rounded-sm border border-breach/40 px-1.5 py-0.5 font-mono text-[12px] uppercase tracking-[0.1em] text-breach-text">
                  Breach
                </span>
              )}
            </li>
