/**
 * HYPOTHESIS SPIKE — inbound (SERVER-side) JSON-RPC surface.
 *
 * This is the mirror image of `src/harness/mcp/protocol.ts`. That file is a
 * CLIENT: it validates the *results* a remote server sends back. Here MCPwn is
 * the server ([ADR-0006](../../../docs/adr/0006-mcpwn-is-the-mcp-server.md)), so
 * we validate the *requests* an agent sends us, and we PRODUCE results.
 *
 * The envelope constants and the Zod discipline are reused from the client
 * layer; the request-handling half is new (there was nothing to reuse — the
 * client never had to read a request).
 *
 * HOSTILE SURFACE. We are deliberately inviting an agent we do not control to
 * speak to this endpoint, so every inbound message is Zod-validated before any
 * field is read, and every failure becomes a TYPED `SpikeProtocolError` that the
 * dispatcher turns into a JSON-RPC error response. Nothing here throws raw.
 *
...
export type SpikeErrorCode =
  ...
  | 'INVALID_REQUEST'
  /** A method outside the spike's four-message surface. */
  | 'METHOD_NOT_FOUND'
  /** The method is served, but its params failed validation. */
  | 'INVALID_PARAMS';

const RPC_CODES: Record<SpikeErrorCode, number> = {
...
export class SpikeProtocolError extends Error {
  readonly code: SpikeErrorCode;
  readonly rpcCode: number;

  constructor(code: SpikeErrorCode, message: string, options?: { cause?: unknown }) {
    super(message, options);
    this.name = 'SpikeProtocolError';
    this.code = code;
...
// ── params (only the three served methods) ──
