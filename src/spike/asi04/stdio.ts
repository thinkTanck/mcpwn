/**
 * HYPOTHESIS SPIKE (ASI04) — stdio transport.
 *
 ...
 * no per-run token — because those are production concerns that bear nothing on
 * the behavioral question. The honest cost: this proves nothing about the
 * hosted-HTTP-plus-token path, which stays a separate, unretired risk.
 *
 * TWO CHANNELS, never crossed. The MCP channel (stdout in production) carries
 * JSON-RPC only — a stray log line there corrupts the stream the client parses.
 * The run log goes to the side channel (stderr, or a file).
 *
 * The streams are injected, so the loop is exercised in-memory by a loopback
 * client under test with no process involved.
 */
import type { Trace } from '@/contract';
import { RPC_PARSE_ERROR, jsonRpcError } from '@/spike/asi04/protocol';
import type { SpikeMcpServer } from '@/spike/asi04/server';
