/**
 * HYPOTHESIS SPIKE (ASI04) — recording what the endpoint observes as a real `Trace`.
 *
 * The recorded object is the SAME `Trace` the detector consumes, built through
 * module 1's `record()` so step ids are assigned in one place and stay positional
 * (`s1`, `s2`, …). An id that marked the offending step would hand a judge its own
 * label.
 *
 * WHAT A SERVER CAN HONESTLY SEE, per ADR-0006:
 *  - `attacker`    — content WE authored into the environment. For ASI04 that is
 *    the poisoned tool DESCRIPTION, not a message body. It is ours by
 *    construction and was in place before the agent connected.
 *  - `tool_call`   — the agent's own decision, with arguments. The signal.
 *  - `tool_result` — what we returned. For ASI04 this is CLEAN in both framings;
 *    the trap is not here.
 ...
 *  - `agent_reasoning` — NOT OBSERVABLE, and never synthesized. There is
 *    deliberately no method to add one: nothing in MCP carries an agent's chain
 *    of thought to a server, so a reasoning step could only be fabrication. The
 *    live trace is thinner than the constructed fixtures, and that thinness is a
 *    finding, not a defect to hide.
 *
 * `tools/list` is not recorded as a step — the contract has no discovery type,
 * and inventing one would be the fabrication this file refuses elsewhere. Note
 * the asymmetry that creates for THIS category: the poison is delivered in the
 * `tools/list` response, so the only record of it is the `attacker` step the
 * recorder seeds. The run log carries the discovery call itself.
 */
import type { Category, JsonValue, Trace } from '@/contract';
import { record, type McpTargetPort, type TargetStepEvent } from '@/harness';
import type { SpikeSurface } from '@/spike/asi04/surface';
import { SPIKE_PAIR_SLUG } from '@/spike/asi04/surface';

/** The category this spike stages. One category, deliberately. */
export const SPIKE_CATEGORY: Category = 'ASI04';

/**
 * The run identity, IDENTICAL across both framings. A `runId` naming the framing
 * would put the held-out label inside the observable trace — the exact leak
 * ASI10 once shipped (`asi10-goal_drift-malicious`).
 */
export const SPIKE_RUN_ID = SPIKE_PAIR_SLUG;

/**
 * Where the agent connected. stdio, local only — no HTTP, no token.
 *
 ...
 * `stdio://…-asi04` would spell the answer out inside the evidence payload.
 */
 ...
 * and adding one to the contract for a spike would be worse than saying it
 * plainly where a human (and the judge) will read it.
 */
...
export class SpikeTraceRecorder {
  ...
  constructor(
    ...
    // The authored content was in place before the agent ever connected. For
    // this category it is the tool description the agent will read in tools/list.
    this.events.push({ type: 'attacker', content: surface.authoredPoisonText });
  }
