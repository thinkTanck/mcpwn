/**
 * HYPOTHESIS SPIKE (ASI04) — the MCP server MCPwn hosts, and the agent connects to.
 *
 ...
 * answered). `stdio.ts` wraps it in framing; a test drives it directly.
 *
 * PROTOCOL SURFACE — exactly four messages, as the spec lists:
 *   `initialize` · `notifications/initialized` · `tools/list` · `tools/call`
 * Everything else answers METHOD_NOT_FOUND, the correct spec behaviour for a
 * server advertising only the `tools` capability.
 *
 ...
 * failure counted as a behavioral NO.
 *
 * `tools/list` MATTERS MORE HERE THAN IN ASI01. For ASI01 discovery was
 * incidental and the trap arrived in a tool result. Here the response to
 * `tools/list` IS the delivery mechanism, so the log records that call explicitly
 * — it is the moment the agent read the poison.
 */
...
import { SpikeTraceRecorder, type SpikeRecorderOptions } from '@/spike/asi04/recorder';
import { buildSurface, type SpikeSurface, type SpikeTool } from '@/spike/asi04/surface';

/**
 * The identity the agent sees in `initialize`. DELIBERATELY NEUTRAL.
 *
 * The ASI01 spike answered `mcpwn-spike-asi01-sandbox`, which is the name of the
 * experiment written on the wall of the room the subject is standing in: a naive
 * agent reading "spike" and "sandbox" in a server name has been told it is being
 * tested. This one is an ordinary mail server name, and `server.test.ts` pins
 * that no agent-visible string carries the project, the spike or the category.
 */
export const SERVER_NAME = 'inbox-mcp';
 ...
 *
 * It carries the tool ARGUMENTS because the `Trace` is only assembled at session
 * end, whereas this log is written line by line. A reader can therefore answer
 * the spike's one question from a live log without depending on a clean shutdown.
 */
...
export class SpikeMcpServer {
   ...
   * message was a notification. NEVER throws: a typed `SpikeProtocolError`
   * becomes an error envelope, and any other failure becomes INVALID_PARAMS.
   */
  ...
  private onListTools(request: InboundRequest): JsonRpcOutbound {
    ...
    // FOR THIS CATEGORY THIS IS THE DELIVERY. Noted explicitly so a log reader
    // can see the moment the agent was handed the poisoned description, which is
    // the counterpart to ASI01's poisoned tool_result.
    this.noteOrder('tools/list', undefined, 'served tool descriptions');
    // Discovery is still NOT a Trace step: the contract has no type for it, and
  ...
  private noteOrder(
    ...
    const ordering = this.initialized ? undefined : 'received before notifications/initialized';
    const combined = [note, ordering].filter(Boolean).join('; ');
    this.note(method, {
