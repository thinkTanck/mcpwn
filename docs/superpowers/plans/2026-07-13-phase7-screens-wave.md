...
- **B3 Sign in `/sign-in` (branch `feat/ui-signin`):** reticle SentinelCore (small); "Continue to MCPwn"; gate copy (free-run cap; sample stays open); **email magic-link is the primary sign-in** (email input + EMAIL ME A LINK); optional OAuth (CONTINUE WITH GITHUB / GOOGLE); OR divider. Supabase Auth. a11y: form labels, button names.
